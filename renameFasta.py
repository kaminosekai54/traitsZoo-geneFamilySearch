...
def fastaToDict(fastaFile):
    if "taraData" in fastaFile: return taraDataToDict(fastaFile)
    if "_" in fastaFile:
        speciesName = fastaFile[fastaFile.rfind("/")+1: fastaFile.find("_")]
    else:
        speciesName = fastaFile[fastaFile.rfind("/")+1: fastaFile.rfind(".")]
    return {speciesName+ "|" + str(i) : record.seq for i, record in enumerate(SeqIO.parse(fastaFile, "fasta"))}

def taraDataToDict(fastaFile):
    if "MGT" in fastaFile : 
        return {record.id.split()[0].replace("_", "|", 1): record.seq for i, record in enumerate(SeqIO.parse(fastaFile, "fasta-2line"))}
    elif "MAG" in fastaFile:
        return {record.id.split()[0][:record.id.find("MAG")+4] + "|" + record.id.split()[0][record.id.find("MAG")+4:]:record.seq for i, record in enumerate(SeqIO.parse(fastaFile, "fasta-2line"))}
...
def fastaToTSV(fastaFile, fastaDict= {}):
    if "_" in fastaFile: speciesName = fastaFile[fastaFile.rfind("/"): fastaFile.find("_")]
    else: speciesName = fastaFile[fastaFile.rfind("/"): fastaFile.rfind(".")]
...
def convertAll():
    t1 = time.time()
    with Pool() as p:
        p.map(processFile, [settings["path"]["rawData"] +file for file in os.listdir(settings["path"]["rawData"]) if os.path.isfile(settings["path"]["rawData"]+file)])
        # p.map(processFile, [settings["path"]["rawData"] +file for file in os.listdir(settings["path"]["rawData"]) if os.path.isfile(settings["path"]["rawData"]+file) and not settings["candidatFile"] in file])
    t2= time.time()
