...
def blastRefToOther(dbFile, querryFile= settings["path"]["renamedFasta"]+ settings["candidatFile"] + "_pep.fasta"):
    if os.path.isfile(querryFile) and os.path.isfile(dbFile):
        ...
        cmd = ["diamond", "blastp", "--very-sensitive", "-q", querryFile, "-d", dbFile, "-o", outputFile, "--outfmt", "6", "qseqid","sseqid","qlen","slen","length","ppos","pident","evalue","bitscore","full_qseq","full_sseq"]
        # print(cmd)
...
def allBlastSearch():
    ...
    t1 = time.time()
    with Pool() as p:
        p.map(blastRefToOther, [settings["path"]["indexedFiles"] + file for file in os.listdir(settings["path"]["indexedFiles"]) if not settings["candidatFile"] in file])
    t2 = time.time()
